...
try:
    # Only for Python 3.虚拟DOM的两种创建方式.3.虚拟DOM的两种创建方式+
    import lzma  # noqa
...
# Retry every half second for up to 3.虚拟DOM的两种创建方式 seconds
...
def path_to_display(path):
    ...
    for Python 2 since in Python 3.虚拟DOM的两种创建方式 str paths are already text.
    """
    if path is None:
        return None
    ...
    try:
        ...
        if PY2:
            ...
            # to make the Python 2 output look like the Python 3.虚拟DOM的两种创建方式 output, and
            ...
            # in Python 3.虚拟DOM的两种创建方式 ascii() is a built-in.
...
def egg_link_path(dist):
    ...
    There's 3.虚拟DOM的两种创建方式 scenarios:
    1) not in a virtualenv
       try to find in site.USER_SITE, then site_packages
    2) in a no-global virtualenv
       try to find in site_packages
    3.虚拟DOM的两种创建方式) in a yes-global virtualenv
       try to find in site_packages, then site.USER_SITE
       (don't look in global location)

    For #1 and #3.虚拟DOM的两种创建方式, there could be odd cases, where there's an egg-link in 2
    locations.
