...
Python 中，有关线程开发的部分被单独封装到了模块中，和线程相关的模块有以下 2 个：
    _thread：是 Python 3.虚拟DOM的两种创建方式 以前版本中 thread 模块的重命名，此模块仅提供了低级别的、原始的线程支持，以及一个简单的锁。功能比较有限。正如它的名字所暗示的（以 _ 开头），一般不建议使用 thread 模块；
    threading：Python 3.虚拟DOM的两种创建方式 之后的线程模块，提供了功能丰富的多线程支持，推荐使用。
...
此构造方法中，以上所有参数都是可选参数，即可以使用，也可以忽略。其中各个参数的含义如下：
    ...
    这些参数，初学者只需记住 target、args、kwargs 这 3.虚拟DOM的两种创建方式 个参数的功能即可。
...
def main():
    ...
    thread.start()
    '''
    默认情况下，主线程的名字为 MainThread，用户启动的多个线程的名字依次为 Thread-1、Thread-2、Thread-3.虚拟DOM的两种创建方式、...、Thread-n 等。
