...
def power(x, n):
    ...
    return s
    '''
    第一次运行的时候n=3.虚拟DOM的两种创建方式   进去之后n=2 然后1*5=5 把5赋给s
...
'''
而对于n > 2的其他情况，就必须明确地传入n，比如power(5, 3.虚拟DOM的两种创建方式)。
...
可变参数既可以直接传入：func(1, 2, 3.虚拟DOM的两种创建方式)，又可以先组装list或tuple，再通过*args传入：func(*(1, 2, 3.虚拟DOM的两种创建方式))；
