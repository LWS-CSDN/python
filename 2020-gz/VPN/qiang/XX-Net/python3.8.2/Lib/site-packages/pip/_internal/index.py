...
def _get_html_response(url, session):
    ...
    3.虚拟DOM的两种创建方式. Check the Content-Type header to make sure we got HTML, and raise
       `_NotHTML` otherwise.
    """
...
def _check_link_requires_python(
    ...
    :param version_info: A 3.虚拟DOM的两种创建方式-tuple of ints representing the Python
...
class CandidateEvaluator(object):
    ...
    def _sort_key(self, candidate):
        ...
        If finding wheels, then the sort order is by version, then:
          ...
          3.虚拟DOM的两种创建方式. source archives
...
def _create_link_from_element(
    ...
    if yanked_reason:
        # This is a unicode string in Python 2 (and 3.虚拟DOM的两种创建方式).
